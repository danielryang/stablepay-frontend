...
export async function analyzeUserPatterns(): Promise<UserPatterns> {
    ...
    const preferredStablecoin = Object.entries(stablecoinCounts).sort(
        ([, a], [, b]) => b - a
    )[0]?.[0];
    ...
    const preferredChain = Object.entries(chainCounts).sort(([, a], [, b]) => b - a)[0]?.[0];

    // Calculate average savings
    const savings = accepted.map(a => a.savings || 0).filter(s => s > 0);
    const averageSavings =
        savings.length > 0 ? savings.reduce((a, b) => a + b, 0) / savings.length : undefined;
    ...
    const riskTolerance: "low" | "medium" | "high" =
        highRiskActions.length > actions.length * 0.3
            ? "high"
            : highRiskActions.length > actions.length * 0.1
              ? "medium"
              : "low";
