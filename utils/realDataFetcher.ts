...
function getCoinGeckoApiKey(): string | null {
    ...
    if (!apiKey) {
        try {
            ...
            apiKey = Constants.expoConfig?.extra?.coingeckoApiKey || 
    ...
    if (apiKey) {
        ...
        console.warn('⚠️ No CoinGecko API key found. Check your .env file and restart Expo server.');
    }
...
export async function fetchRealHistoricalData(days: number = 90): Promise<HistoricalDataResult> {
    ...
    if (apiKey) {
        console.log(`🔑 Using Demo API key: ${apiKey.substring(0, 5)}...${apiKey.substring(apiKey.length - 3)}`);
    } else {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 429) {
                    throw new Error(`Rate limited by CoinGecko API. Please wait a few minutes and try again.`);
                }
                if (response.status === 400) {
                    // Check if error says to use Pro API
                    if (errorDetails && errorDetails.includes('Pro API key') && errorDetails.includes('pro-api.coingecko.com')) {
                        ...
                        if (!proResponse.ok) {
                            ...
                            throw new Error(`Pro API also failed for ${chain.name}: ${proErrorText.substring(0, 200)}`);
                        }
                        
                        // Use Pro API response
                        const proData = await proResponse.json();
                        historicalData[chain.name] = proData.prices.map((point: [number, number], idx: number) => {
                        ...
                        console.log(`✅ Fetched ${historicalData[chain.name]?.length} days for ${chain.name} (via Pro API)`);
                        await new Promise(resolve => setTimeout(resolve, 3000));
                        continue; // Skip to next chain
                    }
                    
                    // Check if error says to use Demo API (regular endpoint)
                    if (errorDetails && errorDetails.includes('Demo API key') && errorDetails.includes('api.coingecko.com')) {
                ...
                if (response.status === 401) {
                    throw new Error(`Unauthorized: Invalid API key. Please check your EXPO_PUBLIC_COINGECKO_API_KEY in .env file.`);
                }
                throw new Error(`Failed to fetch ${chain.name} data: ${errorMessage}${errorDetails ? '. ' + errorDetails : ''}`);
            }
            ...
            if (chain.name === 'ethereum' || chain.name === 'polygon') {
                throw new Error(`Failed to fetch required chain data for ${chain.name}. ${error instanceof Error ? error.message : 'Unknown error'}`);
            }
...
async function fetchChainLiquidityFromDefiLlama(): Promise<{
    ...
    try {
        ...
        if (!response || !response.ok) {
            console.warn(`⚠️ All DefiLlama endpoints failed (last error: ${lastError}), using fallback liquidity scores`);
            return null;
        }
        ...
        if (assets && assets.length > 0) {
            for (const asset of assets) {
                ...
                for (const [chainName, balance] of Object.entries(chainData)) {
                    ...
                    const normalizedChainName = chainName.charAt(0).toUpperCase() + chainName.slice(1).toLowerCase();
                    ...
                    if (mappedChain) {
                        ...
                        if (typeof balance === 'number') {
                            ...
                            balanceValue = (balance as any).circulating?.peggedUSD 
 ...
 */
async function fetchPlatformData(coinId: string, apiKey: string | null): Promise<{ [chain: string]: string } | null> {
    try {
        const headers: HeadersInit = {
            'Accept': 'application/json',
        };
        ...
        const response = await fetch(url, {
            ...
            if (error.message?.includes('CORS') || error.message?.includes('Failed to fetch')) {
                console.warn(`⚠️ CORS error fetching platform data for ${coinId} (browser restriction). Continuing without platform data.`);
                return null;
            }
...
function calculateLiquidityScores(
    ...
    if (!chainLiquidity) {
        ...
        return fallbackScores[stablecoin] || { ethereum: 0.5, polygon: 0.5, arbitrum: 0.5, solana: 0.5 };
    }
...
function estimateConversionFees(
    ...
    const baseFee = marketCap > 50_000_000_000 && volume24h > 1_000_000_000
...
export async function fetchStablecoinLiquidityData(): Promise<{
    ...
    try {
        ...
        if (!priceResponse.ok) {
            ...
            throw new Error(`Failed to fetch stablecoin liquidity data: HTTP ${priceResponse.status}`);
        }
        ...
        for (const coin of coinIds) {
            try {
                ...
                console.warn(`⚠️ Could not fetch platform data for ${coin.name}, continuing without it`);
...
export function analyzeOptimalTiming(
    ...
    const avgByDay: { [day: string]: { day: number; dayName: string; avgCost: number; sampleSize: number } } = {};
...
export async function fetchFiatExchangeRate(fiatCurrency: string): Promise<FiatExchangeRate> {
    ...
    try {
        ...
        if (data.rates && data.rates[fiatCode]) {
            ...
            if (rate > 100000) {
                console.warn(`⚠️ Exchange rate seems inverted (${rate}). Trying inverse calculation...`);
            ...
            if (rate < 100 || rate > 10000) {
                console.warn(`⚠️ Exchange rate ${rate} seems invalid for ${fiatCurrency}. Using fallback...`);
        ...
        if (fallbackResponse.ok) {
            ...
            if (fallbackData['usd-coin'] && fallbackData['usd-coin'][fiatCode]) {
                ...
                console.log(`✅ Fetched exchange rate (fallback via USDC): 1 USD ≈ ${rate} ${fiatCurrency}`);
        ...
        if (fiatCode === 'ars') {
            ...
            console.warn(`⚠️ Using estimated ARS rate (${estimatedRate} ARS = 1 USD). Consider using a real exchange rate API.`);
            return {
                fiatCurrency: 'ARS',
        ...
        throw new Error(`Exchange rate not found for ${fiatCurrency}. Please check CoinGecko API documentation.`);
    } catch (error: any) {
        console.error(`Error fetching fiat exchange rate for ${fiatCurrency}:`, error);
        throw new Error(`Failed to fetch exchange rate for ${fiatCurrency}: ${error.message || 'Unknown error'}`);
    }
}
